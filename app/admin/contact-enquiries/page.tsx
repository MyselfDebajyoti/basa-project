...
export default function ContactEnquiriesPage() {
  const [data, setData] = useState<ContactEnquiriesFormDocument[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      setData(response.documents as ContactEnquiriesFormDocument[]);
    } catch (error) {
  ...
  const downloadExcel = () => {
    try {
      ...
      const fileName = `contact_enquiries_${
  ...
  const deleteSingleRow = async (docId: string, name: string) => {
    if (!confirm(`Are you sure you want to delete the contact enquiry from "${name}"? This action cannot be undone.`)) {
      return;
    }

    try {
      setDeleteLoading(docId);
      await databases.deleteDocument(DATABASE_ID!, TABLES.CONTACT_ENQUIRIES_FORM, docId);
  ...
  const deleteAllData = async () => {
    try {
      ...
      const deletePromises = data.map(doc => 
        databases.deleteDocument(DATABASE_ID!, TABLES.CONTACT_ENQUIRIES_FORM, doc.$id)
      );
      ...
      alert(`Successfully deleted all ${data.length} contact enquiries!`);
    } catch (error) {
      console.error("Error deleting all data:", error);
      alert("Error deleting all data. Some records may not have been deleted. Please refresh and try again.");
    } finally {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-4 md:col-span-2 lg:col-span-3">
          <div className="text-xs text-gray-600 mb-2">Contact Form Link</div>
          <div className="flex items-center gap-2">
            <input
              type="text"
              value="https://www.amaderbasa.org/contact"
              readOnly
            ...
            <button
              onClick={() => {
                navigator.clipboard.writeText(
                  "https://www.amaderbasa.org/contact"
                );
            ...
            <button
              onClick={() => {
                const shareUrl = "https://www.amaderbasa.org/contact";
                if (navigator.share) {
                  navigator
                    .share({
